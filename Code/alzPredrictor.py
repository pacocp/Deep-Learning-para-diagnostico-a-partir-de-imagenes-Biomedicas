...
class MainWindow():
	...
	def __init__(self, main,filename, df_results):
		...
		self.button_chooseImage.place(x=(screen_width/2)-120,y=0)

		# button to change to predict image
		self.button_predict = Button(main, text="Predict", command=self.predict(filename))
		#self.button.grid(row=4, column=11)
		""" You can change the place of the button giving
		different values to the screen_width and screen_height """
		self.button_predict.place(x=(screen_width/2),y=0)
	...
	def predict(self,filename):
		...
		data_of_training = {'ID': len(self.df_results.index),'NAME': filename, 'PREDICTED LABEL': prediction[0]}
